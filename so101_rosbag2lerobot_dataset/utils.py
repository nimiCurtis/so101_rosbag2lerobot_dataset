...
def ros_image_to_chw_float01(msg) -> np.ndarray:
    enc = msg.encoding
    ...
    return (arr.astype(np.float32) / 255.0).transpose(2, 0, 1)  # CHW


def jointstate_to_vec6(js_msg, joint_order: Optional[List[str]] = None) -> np.ndarray:
...
def float64multiarray_to_vec6(arr_msg, size: int = 6) -> np.ndarray:
    """
